...
delta=0.3; % dimensionless change in height
...
vv=[0.75, 1, 1.25, 1.5, 1.75]; % values of h-to-a ratio
%vv=[0.33 0.5 0.625 0.75, 1, 1.25, 1.5, 1.75]; % values of h-to-a ratio

%dth=0 % overtwist (degrees)

dth=-10; % overtwist value (degrees)
...
dd=zeros(size(vv)); % vectors of delta parameters (initialization)
dd2=zeros(size(vv));
...
lgd=legend(axh,num2str(vv','%1.2f'),'Location','Southeast');
disp([' ']); disp(['Figure: force-vs-displacement plot']);
lgd=legend(axh2,num2str(dd','%1.3f'),'Location','Southeast');


saveas(axh,'fig_kn_h2a_bs.fig')
saveas(axh,'fig_kn_h2a_bs.pdf')



return

saveas(axh,'fig_kn_b2av.pdf')
saveas(axh2,'fig_kn_b2av_delta.pdf')

% % show deformed configuration
% figure
% modepicture(nodes(:)-nodes0(:),1,[1 1 1],nodes0(:),edges,stypes)
% set(gca,'clipping','off')
% set(gcf,'name','Final configuration');
% view(myview);
% set(gca,'color','w'); set(gcf,'color','w');
% set(gca,'clipping','off');
% disp([' ']); disp(['Figure: Final configuration']);
% disp(['Press a key to continue or CTRL+C to exit.']);
% disp([' ']);
% pause

...
Nsteps=50; % number of incremental steps

delta=0.1; % dimensionless change in height

y1=-0.0015; y2=0.0025; %ylim
...
ksr=1*0.0001; % angular spring constant scaling
...
lgd=legend(axh,num2str(vv','%1.2f'),'Location','Northwest');
disp([' ']); disp(['Figure: force-vs-displacement plot']);
lgd=legend(axh2,num2str(dd','%1.3f'),'Location','Southwest');

saveas(axh,'fig_kn_b2a_ps.fig')
saveas(axh,'fig_kn_b2a_ps.pdf')

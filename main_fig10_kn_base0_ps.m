...
ksr=0.000; % angular spring constant scaling
...
nodesg=nodes;
    ...
    eqlforcesm2 = zeros(3*Nnodes,Nsteps); % balanced load matrix initialization
    
    estrainm = zeros(Nedges,Nsteps+1); % axial force matrix initialization
    estrainm2 = zeros(Nedges,Nsteps+1); % axial force matrix initialization
    estressm = zeros(Nedges,Nsteps+1); % axial force matrix initialization
    
    estrainmax=zeros(size(epsv,2),1);
    estrainmin=zeros(size(epsv,2),1);
    estrainmax2=zeros(size(epsv,2),1);
    estrainmin2=zeros(size(epsv,2),1);
 

%% main loop

for kk=1:size(epsv,2)
...
%         +eqlmatz(nodes,zedges)*zstress;
    ...
    for ii=1:Nsteps
        ...
        eqlforcesm2(:,ii)=eqlforces; % save nodal forces
        
%         estressm(:,ii)=estress; % save axial forces
%         
        l2=length0(nodes,edges); % final length
        estrainm2(:,ii)=(l2-lf)./lf;
...
estrainmax(kk) = max(max(estrainm));
estrainmax2(kk) = max(max(estrainm2));
estrainmin(kk) = min(min(estrainm));
estrainmin2(kk) = min(min(estrainm2));
  
%% post-processing

% force-vs-displacement plot
plot(axh,...
    [(((Nsteps:-1:1)/Nsteps)*(-delta)) 0 (((1:Nsteps)/Nsteps)*delta)],...
    [n*eqlforcesm2(3,end:-1:1)/kc/a, 0, n*eqlforcesm(3,:)/kc/a],...
    'linewidth',2);

set(axh,'xlim',[x1,x2],'ylim',[y1,y2]);
...
set(axh2,'xlim',[x1,x2],'ylim',[y1,y2]);
...
%[estrainmax estrainmin estrainmax2 estrainmin2]

lgd=legend(axh,num2str(epsv','%1.3f'),'Location','Northwest');
disp([' ']); disp(['Figure: force-vs-displacement plot']);
lgd=legend(axh2,num2str(dd','%1.3f'),'Location','Northwest');


return

saveas(axh,'fig_kn_base0.pdf')
saveas(axh2,'fig_kn_base0_delta.pdf')


% saveas(axh,'fig_kn_epsv.pdf')
% saveas(axh2,'fig_kn_epsv_delta.pdf')

...
function [nodes,edges,wedges,stypes,wtypes]=genmodtnw(n,a,b,h,dth,or)
...
Nwedges=2*n;
wedges=zeros(Nwedges,3);
wtypes=zeros(Nwedges,1);
for k=1:n
   ...
   stypes(k+3*n)=3;
   wedges(k,:)=[k 1+mod(k,n) n+1+mod(k+n-2,n)];
   wtypes(k)=1;
   wedges(n+k,:)=[n+k 1+mod(k,n) n+1+mod(k+n-2,n)];
   wtypes(n+k)=2;
   wedges(2*n+k,:)=[k k+n 1+mod(k+1,n)];
   wtypes(2*n+k)=3;
   wedges(3*n+k,:)=[k+n k n+1+mod(k,n)];
   wtypes(3*n+k)=4;
end
